...
namespace Lab_3_4
{
    class FileQueue
    {
        ...
        public static void studSplit2(Queue<studentas> geresni, Queue<studentas> blogesni)
        {
            File.Create(@"d:\Geresni.txt").Close();
            File.Create(@"d:\Blogesni.txt").Close();
            using (StreamWriter ger = new StreamWriter(@"d:\Geresni.txt"))
        ...
        public static void queue()
        {
            ...
            Queue<studentas> geresni = new Queue<studentas>();
            ...
            foreach (var student in studentai)
            {
                if (student.vidurkis > 5)
                {
                    geresni.Enqueue(new studentas { vardas = student.vardas, pavarde = student.pavarde, vidurkis = student.vidurkis, mediana = student.mediana });
                }
                else
                {
                    blogesni.Enqueue(new studentas { vardas = student.vardas, pavarde = student.pavarde, vidurkis = student.vidurkis, mediana = student.mediana });
                }
            }
            watch1.Stop();
